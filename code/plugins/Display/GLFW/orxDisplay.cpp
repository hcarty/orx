...
static orxINLINE GLFWmonitor *orxDisplay_GLFW_GetMonitor()
{
  ...
  if(apstMonitors != NULL)
  {
    ...
    /* Gets config monitor */
    s32Monitor = orxConfig_GetS32(orxDISPLAY_KZ_CONFIG_MONITOR);

    /* Valid? */
    if(s32Monitor > 0)
    {
      /* Is index in range? */
      if(s32Monitor <= iCount)
      {
        /* Updates result */
        pstResult = apstMonitors[s32Monitor - 1];
      }
      else
      {
        /* Logs message */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY, "Invalid monitor: index [%d] out of range, max is [%d]. Reverting to current monitor.", s32Monitor, iCount);
      }
    }
    /* Should hide? */
    else if(s32Monitor < 0)
    {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN, orxDISPLAY_KU32_STATIC_FLAG_NONE);
    }
    ...
    if(pstResult == NULL)
    {
      ...
      /* Not hidden? */
      if(!orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN))
      {
        /* Stores it */
        orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_MONITOR, (orxU32)s32Monitor);
      }
    }
...
{
  /* Not ignoring event, not hidden and not fullscreen? */
  if(!orxFLAG_TEST(sstDisplay.u32Flags, (orxDISPLAY_KU32_STATIC_FLAG_IGNORE_EVENT | orxDISPLAY_KU32_STATIC_FLAG_HIDDEN | orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN)))
  {
    orxVECTOR     vPosition;
    GLFWmonitor **apstMonitors;
    orxU32        u32Monitor = 1;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_TransformText(const orxSTRING _zString, const orxBITMAP *_pstFont, const orxCHARACTER_MAP *_pstMap, const orxDISPLAY_TRANSFORM *_pstTransform, orxRGBA _stColor, orxDISPLAY_SMOOTHING _eSmoothing, orxDISPLAY_BLEND_MODE _eBlendMode)
{
  ...
  /* Inits matrix */
  orxDisplay_GLFW_InitMatrix(&mTransform, _pstTransform, _pstFont);
...
orxBITMAP *orxFASTCALL orxDisplay_GLFW_LoadFont(const orxSTRING _zFileName, const orxSTRING _zCharacterList, const orxVECTOR *_pvCharacterSize, const orxVECTOR *_pvCharacterSpacing, const orxVECTOR *_pvCharacterPadding, orxBOOL _bSDF, orxFLOAT *_afCharacterWidthList)
{
  orxBITMAP *pstResult = orxNULL;
                ...
                if(stbtt_InitFont(&(pstLoadInfo->stFontInfo), pu8Buffer, 0) != 0)
                {
                  ...
                  if(pstLoadInfo->u32GlyphCount > 0)
                  {
                    ...
                    if(pstLoadInfo->astGlyphList != orxNULL)
                    {
                      ...
                      {
                        /* Processes font */
                        if(orxDisplay_GLFW_ProcessFont(pstLoadInfo) != orxSTATUS_FAILURE)
                        {
                          /* Uploads texture */
                          orxDisplay_GLFW_DecompressBitmapCallback(pstLoadInfo);
                        }
                      }
                    }
                    else
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetVideoMode(const orxDISPLAY_VIDEO_MODE *_pstVideoMode)
{
  ...
  if(_pstVideoMode != orxNULL)
  {
    ...
    {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN);
    }
    ...
    /* Has a window or same depth & visibility? */
    if((sstDisplay.pstWindow != orxNULL)
    && ((orxU32)iDepth == sstDisplay.pstScreen->u32Depth)
    && (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN) != glfwGetWindowAttrib(sstDisplay.pstWindow, GLFW_VISIBLE)))
    {
    ...
    else
    {
      ...
      glfwWindowHint(GLFW_VISIBLE, orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN) ? GLFW_FALSE : GLFW_TRUE);
